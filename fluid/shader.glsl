...
float[fraction_size][10] getVoxel(uint chunkNeighbours[3][3][3], uint x, uint y, uint z){
	uint currentChunk = chunkNeighbours
		[chunkOffset(gl_LocalInvocationID.x+x)]
		[chunkOffset(gl_LocalInvocationID.y+y)]
		[chunkOffset(gl_LocalInvocationID.z+z)];
	if (currentChunk >= chunkDataBuffer.data.length()) {
		return defaultVoxelBuffer.data;
	}
	return chunkDataBuffer.data[currentChunk]
		[(int(gl_LocalInvocationID.x+x)-1)%chunk_size]
		[(int(gl_LocalInvocationID.y+y)-1)%chunk_size]
		[(int(gl_LocalInvocationID.z+z)-1)%chunk_size];
}
...
void setVoxel(uint chunkNeighbours[3][3][3], uint x, uint y, uint z, float data[fraction_size][10]){
	...
	chunkDataBuffer.data[currentChunk]
		[(int(gl_LocalInvocationID.x+x)-1)%chunk_size]
		[(int(gl_LocalInvocationID.y+y)-1)%chunk_size]
		[(int(gl_LocalInvocationID.z+z)-1)%chunk_size] = data;
}
			...
			for (uint z = 0; z<3; z++){
				...
				for (uint w = 0; w<fraction_size; w++){
					vec3 currentPosition = vec3(
						currentVoxel[w][3]+float(x)-1.0,
						currentVoxel[w][4]+float(y)-1.0,
						currentVoxel[w][5]+float(z)-1.0);
...
void recalculateVelocities(uint chunkNeighbours[3][3][3]){
	...
	for (uint i=0; i<fraction_size; i++){
		...
		fix += calculateFractionMass(currentVoxel[i]) *
			calculatePressure(
				chunkNeighbours,
				vec3(currentVoxel[i][3], currentVoxel[i][4], currentVoxel[i][5]),
				currentVoxel[i][9]);
...
uint swapFullness(uint chunkNeighbours[3][3][3], uint x, uint y, uint z, uint data){
	...
	return atomicExchange(
		fullnessBuffer.data[currentChunk]
			[(int(gl_LocalInvocationID.x+x)-1)%chunk_size]
			[(int(gl_LocalInvocationID.y+y)-1)%chunk_size]
			[(int(gl_LocalInvocationID.z+z)-1)%chunk_size],
		data);
...
void main() {
			...
			for (uint i = 0; i < fraction_size; i++) {
				if (previousVoxel[i][9]!=0.0){
					...
					for (uint j = 0; j < voxelCount && !simmilar; j++){
						if (currentVoxel[j][0]==previousVoxel[i][0] &&
							currentVoxel[j][1]==previousVoxel[i][1] &&
							currentVoxel[j][2]==previousVoxel[i][2]) {
							if (dot(vec3(currentVoxel[j][6], currentVoxel[j][7], currentVoxel[j][8]),
								vec3(previousVoxel[i][6], previousVoxel[i][7], previousVoxel[i][8]))>=0.1) {
	...
	if (op_mode>0) {
		...
		for (uint i = 0; i < fullnessBuffer.data[chunkNeighbours[1][1][1]][gl_LocalInvocationID.x][gl_LocalInvocationID.y][gl_LocalInvocationID.z]; i++) {
